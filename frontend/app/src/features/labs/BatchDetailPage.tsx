...
export function BatchDetailPage() {
  ...
  if (isLoading) return <Page><div style={{ padding: 40, display: 'flex', justifyContent: 'center' }}><Spinner label="Loading batch…" /></div></Page>;
  if (isError || !data) return <Page><ErrorState message="This batch could not be loaded." action={<Button size="sm" variant="secondary" onClick={() => refetch()}>Retry</Button>} /></Page>;
    ...
    <Page max={1000}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24, gap: 16, flexWrap: 'wrap' }}>
        <div>
          <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 6 }}>
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--fs-13)', color: 'var(--ink-3)' }}>Batch {batch.id.slice(0, 8)}</span>
            <StatusPill status={batch.status} />
          </div>
          <h1 style={{ fontSize: 'var(--fs-30)', margin: 0 }}>{batch.processed_items} / {batch.total_items} processed</h1>
          <div style={{ fontSize: 'var(--fs-13)', color: 'var(--ink-3)', marginTop: 6 }}>Created {formatDate(batch.created_at)}</div>
        </div>
      </div>
      ...
      <Card padding={0}>
        <div style={{ display: 'grid', gridTemplateColumns: '40px 1fr 200px 130px 32px', gap: 16, padding: '10px 16px', background: 'var(--paper)', fontSize: 'var(--fs-12)', color: 'var(--ink-3)', fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.06em', borderBottom: '1px solid var(--line)' }}>
          <div>#</div><div>Image key</div><div>Top candidate</div><div>Status</div><div />
        </div>
        {items.map((it, i) => {
