...
export function NotificationsPage() {
  ...
  return (
    <Page max={760}>
      ...
      <Card padding={0}>
        {isLoading ? (
          <div style={{ padding: 40, display: 'flex', justifyContent: 'center' }}><Spinner label="Loading…" /></div>
        ) : isError ? (
          <ErrorState message="Could not load notifications." action={<Button size="sm" variant="secondary" onClick={() => refetch()}>Retry</Button>} />
        ) : !data || data.length === 0 ? (
          ...
          data.map((n, i) => (
            ...
            >
              ...
              {!n.read && (
                <Button size="sm" variant="ghost" onClick={() => markRead.mutate(n.id)} title="Mark as read">
                  <Check size={14} />
                </Button>
              )}
            </div>
