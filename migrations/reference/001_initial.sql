...
CREATE TABLE reference.syndromes (
...
CREATE TABLE reference.hpo_terms (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE reference.syndrome_hpo (
...
CREATE INDEX idx_syndromes_name ON reference.syndromes(name);
CREATE INDEX idx_hpo_terms_hpo_id ON reference.hpo_terms(hpo_id);
CREATE INDEX idx_hpo_terms_name ON reference.hpo_terms(name);
