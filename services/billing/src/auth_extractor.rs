//! Axum request extractor for JWT bearer tokens used by the billing service.
//!
...
fn decode_token(config: &Config, token: &str) -> Result<Claims, String> {
    // Attempt RS256 first; if the key looks like a PEM RSA public key.
        ...
        || config.jwt_public_key.contains("BEGIN RSA PUBLIC KEY")
    {
        ...
        // Fall back to HS256 with the key value used as a secret.
